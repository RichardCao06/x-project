...
id: A004
node_type: activity
display_name: "SMT贴装, 回流焊接 | GPU加速卡, AI推理用"
boundary: foreground
facets: {transformation_verb: "smt_assembly", technology_route: "reflow_smt", reference_product_anchor: "ref::GPU加速卡, AI推理用"}
reference_product: "GPU加速卡, AI推理用"
produces: [P025, P030, P031]
consumes: [P036, P037, P040, P043, P044, P045, P050, P062, P066, P080, P081]
external: {}
confidence: core
...
spine_hash: "2908c47405c6"
schema_version: wiki-v1
sigil: "sigil/A004.svg"
body_status: draft
...
该活动表示面向AI推理用GPU加速卡的表面贴装与回流焊接装联工序。 〔未核实·模型回忆〕

## 参考产品与参考单位

参考产品锚定为“GPU加速卡, AI推理用”；建模时以一块完成装联的推理用GPU加速卡作为参考单位。 〔未核实·模型回忆〕 [^internal-review]

## 单元过程边界

单元过程边界覆盖GPU加速卡的元器件贴装、焊膏回流焊接及现场工艺辅助投入，不延伸至芯片制造、板材制造或下游系统集成。 〔未核实·模型回忆〕 [^internal-review]

## 技术路线与相邻活动区分

回流SMT路线通过贴装和受控加热使焊膏形成焊点，区别于后续的散热器装配、整卡测试和系统集成活动。 〔未核实·模型回忆〕

## 投入产出与脊边对账

该节点按冻结连接关系消耗P036、P040、P043、P044、P080、P081、P062、P066、P037、P050、P045，并产出P025、P030、P031。 〔未核实·模型回忆〕 [^internal-review]
...
冻结档案关联了4项LCA资料，但尚缺AI推理用GPU加速卡实际BOM、良率、能耗和现场排放的节点特定数据。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  P036["GPU封装器件, AI训练用"] --> A004
  P037["GPU封装器件, AI推理用"] --> A004
  P040["DRAM内存封装器件"] --> A004
  P043["HDI PCB裸板, 服务器/交换机用"] --> A004
  P044["被动元件, 电阻(片式)"] --> A004
  P045["连接器, 板对板"] --> A004
  P050["电源管理IC封装器件"] --> A004
  P062["无铅焊料锡膏, SAC305"] --> A004
  P066["中压电力, ICT制造用"] --> A004
  P080["被动元件, 电容(MLCC)"] --> A004
  P081["被动元件, 电感(片式)"] --> A004
  A004(("SMT贴装, 回流焊接 | GPU加速卡, AI推理用"))
  A004 ==> P025["GPU加速卡, AI推理用"]
  A004 --> P030["共生焊料浮渣"]
  A004 --> P031["共生报废PCBA"]
```
...
|---|---|---|---|---|
| [mounting, surface mount technology, Pb-free solder](https://ecoquery.ecoinvent.org/3.12/cutoff/dataset/5084/documentation) | 源 `m2`；目标 `1 piece GPU加速卡, AI推理用`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
| [Assembly line SMD (1SP, 2CS, 1CP, 1R, 1Rf) throughput 300/h - open input printed circuit board](https://lcadatabase.sphera.com/2026/xml-data/processes/309288a5-19a4-4efc-9e62-e5cf173751cc.xml) | 源 `kg`；目标 `1 piece GPU加速卡, AI推理用`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
