...
id: A001
node_type: activity
display_name: "SMT贴装, 回流焊接 | 主板PCBA, 通用服务器用"
boundary: foreground
facets: {transformation_verb: "smt_assembly", technology_route: "reflow_smt", reference_product_anchor: "ref::主板PCBA, 通用服务器用"}
reference_product: "主板PCBA, 通用服务器用"
produces: [P018, P030, P031]
consumes: [P035, P040, P043, P044, P045, P049, P050, P062, P066, P080, P081, P082, P086, P087]
external: {}
confidence: core
...
spine_hash: "dadca68d566b"
schema_version: wiki-v1
sigil: "sigil/A001.svg"
body_status: draft
...
该活动表示面向通用服务器主板PCBA的表面贴装与回流焊接装联工序。 〔未核实·模型回忆〕

## 参考产品与参考单位

参考产品锚定为“主板PCBA, 通用服务器用”；建模时以一块完成装联的主板PCBA作为参考单位。 〔未核实·模型回忆〕 [^internal-review]

## 单元过程边界

单元过程边界覆盖元器件贴装、焊膏回流焊接及其现场工艺辅助投入，不延伸至上游元器件制造或下游整机装配。 〔未核实·模型回忆〕 [^internal-review]
...
该节点按冻结连接关系消耗P043、P035、P040、P044、P080、P081、P062、P066、P050、P049、P045、P082、P086、P087，并产出P018、P030、P031。 〔未核实·模型回忆〕 [^internal-review]
...
冻结档案关联了4项LCA资料，但尚缺该通用服务器主板实际BOM、良率、能耗和现场排放的节点特定数据。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  P035["CPU封装器件, 服务器用"] --> A001
  P040["DRAM内存封装器件"] --> A001
  P043["HDI PCB裸板, 服务器/交换机用"] --> A001
  P044["被动元件, 电阻(片式)"] --> A001
  P045["连接器, 板对板"] --> A001
  P049["IC封装器件, NIC网络控制器"] --> A001
  P050["电源管理IC封装器件"] --> A001
  P062["无铅焊料锡膏, SAC305"] --> A001
  P066["中压电力, ICT制造用"] --> A001
  P080["被动元件, 电容(MLCC)"] --> A001
  P081["被动元件, 电感(片式)"] --> A001
  P082["卡槽连接器, PCIe用"] --> A001
  P086["卡槽连接器, DIMM内存用"] --> A001
  P087["板载存储连接器, SATA/SAS/U.2用"] --> A001
  A001(("SMT贴装, 回流焊接 | 主板PCBA, 通用服务器用"))
  A001 ==> P018["主板PCBA, 通用服务器用"]
  A001 --> P030["共生焊料浮渣"]
  A001 --> P031["共生报废PCBA"]
```
...
|---|---|---|---|---|
| [mounting, surface mount technology, Pb-free solder](https://ecoquery.ecoinvent.org/3.12/cutoff/dataset/5084/documentation) | 源 `m2`；目标 `1 piece 主板PCBA, 通用服务器用`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
| [Assembly line SMD (1SP, 2CS, 1CP, 1R, 1Rf) throughput 300/h - open input printed circuit board](https://lcadatabase.sphera.com/2026/xml-data/processes/309288a5-19a4-4efc-9e62-e5cf173751cc.xml) | 源 `kg`；目标 `1 piece 主板PCBA, 通用服务器用`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
