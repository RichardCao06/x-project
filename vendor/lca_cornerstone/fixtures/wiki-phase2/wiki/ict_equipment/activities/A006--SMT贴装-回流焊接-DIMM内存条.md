...
id: A006
node_type: activity
display_name: "SMT贴装, 回流焊接 | DIMM内存条"
boundary: foreground
facets: {transformation_verb: "smt_assembly", technology_route: "reflow_smt", reference_product_anchor: "ref::DIMM内存条"}
reference_product: "DIMM内存条"
produces: [P026, P030, P031]
consumes: [P040, P043, P044, P062, P066, P080, P081]
external: {}
confidence: core
...
spine_hash: "d53133ac6272"
schema_version: wiki-v1
sigil: "sigil/A006.svg"
body_status: draft
...
该节点将DIMM内存条的SMT贴装与回流焊接定义为一个前景制造活动。 〔未核实·模型回忆〕 [^internal-review]

## 参考产品与参考单位

参考产品锚定为DIMM内存条，参考单位应以一个已完成贴装和回流焊接的DIMM内存条制造输出定义。 〔未核实·模型回忆〕 [^internal-review]

## 单元过程边界

单元过程边界包括来料PCB与电子元件的表面贴装和回流焊接，不包括上游元器件制造及下游整机集成。 〔未核实·模型回忆〕 [^internal-review]

## 技术路线与相邻活动区分

该节点以回流SMT路线区分于通孔装配、波峰焊、选择性焊接和整机系统集成活动。 〔未核实·模型回忆〕

## 投入产出与脊边对账

投入端应对账PCB、电子元件、焊接材料、能源与辅助材料；产出端应对账DIMM内存条及节点登记的共产品或副产物流。 〔未核实·模型回忆〕 [^internal-review]
...
该节点已有ICT制造过程相关资料线索，但DIMM专用贴装工艺的供应商级前景数据仍需补充。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  P040["DRAM内存封装器件"] --> A006
  P043["HDI PCB裸板, 服务器/交换机用"] --> A006
  P044["被动元件, 电阻(片式)"] --> A006
  P062["无铅焊料锡膏, SAC305"] --> A006
  P066["中压电力, ICT制造用"] --> A006
  P080["被动元件, 电容(MLCC)"] --> A006
  P081["被动元件, 电感(片式)"] --> A006
  A006(("SMT贴装, 回流焊接 | DIMM内存条"))
  A006 ==> P026["DIMM内存条"]
  A006 --> P030["共生焊料浮渣"]
  A006 --> P031["共生报废PCBA"]
```
...
|---|---|---|---|---|
| [mounting, surface mount technology, Pb-free solder](https://ecoquery.ecoinvent.org/3.12/cutoff/dataset/5084/documentation) | 源 `m2`；目标 `1 piece DIMM内存条`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
| [Assembly line SMD (1SP, 2CS, 1CP, 1R, 1Rf) throughput 300/h - open input printed circuit board](https://lcadatabase.sphera.com/2026/xml-data/processes/309288a5-19a4-4efc-9e62-e5cf173751cc.xml) | 源 `kg`；目标 `1 piece DIMM内存条`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
