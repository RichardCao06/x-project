...
id: A031
node_type: activity
display_name: "SMT贴装, 回流焊接 | 存储背板子总成"
boundary: foreground
facets: {transformation_verb: "smt_assembly", technology_route: "reflow_smt", reference_product_anchor: "ref::存储背板子总成"}
reference_product: "存储背板子总成"
produces: [P028, P030, P031]
consumes: [P043, P044, P045, P062, P066, P080, P081, P087]
external: {}
confidence: core
...
spine_hash: "634407fc73a8"
schema_version: wiki-v1
sigil: "sigil/A031.svg"
body_status: draft
...
该活动表示将电子元件贴装至存储背板印制电路板并经回流焊形成已装联背板子总成的制造步骤。 〔未核实·模型回忆〕

## 参考产品与参考单位

参考产品锚定为存储背板子总成，建模时应以一个功能合格并完成回流焊接的子总成为参考单位。 〔未核实·模型回忆〕 [^internal-review]

## 单元过程边界

单元过程边界应覆盖锡膏印刷、元件贴装、回流焊及其现场辅助能源和工艺耗材，不将上游元件制造并入该活动。 〔未核实·模型回忆〕 [^internal-review]

## 技术路线与相邻活动区分

该节点以回流SMT为技术路线，应与仅完成裸板制造、选择性焊接以及整机系统集成活动区分。 〔未核实·模型回忆〕 [^internal-review]

## 投入产出与脊边对账

投入产出对账应以冻结档案所列P043、P044、P080、P081、P062、P066、P045和P087为投入，并以P028、P030和P031为产出，不增删脊边。 〔未核实·模型回忆〕 [^internal-review]

## 直接排放、废物与监测指标边界

应在过程层记录回流焊相关废气控制边界、锡膏和清洗残余物及不合格装联板的去向，但不在本提名阶段给出排放或废物数值。 〔未核实·模型回忆〕 [^internal-review]

## 节点特定采集字段

节点特定采集应包括背板面积或层数、贴装元件类别与数量、锡膏用量、回流炉能源、良率及返工率。 〔未核实·模型回忆〕

## 区域化补充要求

区域化补充应识别制造地点的电力供应、焊接废气治理要求及危险废物处置路径。 〔未核实·模型回忆〕 [^internal-review]

## 数据适用状态与缺口

现有档案关联了ICT生命周期清单来源，但仍需以具体工厂的背板设计、设备能耗、良率和废物流数据确认代表性。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  P043["HDI PCB裸板, 服务器/交换机用"] --> A031
  P044["被动元件, 电阻(片式)"] --> A031
  P045["连接器, 板对板"] --> A031
  P062["无铅焊料锡膏, SAC305"] --> A031
  P066["中压电力, ICT制造用"] --> A031
  P080["被动元件, 电容(MLCC)"] --> A031
  P081["被动元件, 电感(片式)"] --> A031
  P087["板载存储连接器, SATA/SAS/U.2用"] --> A031
  A031(("SMT贴装, 回流焊接 | 存储背板子总成"))
  A031 ==> P028["存储背板子总成"]
  A031 --> P030["共生焊料浮渣"]
  A031 --> P031["共生报废PCBA"]
```
...
|---|---|---|---|---|
| [mounting, surface mount technology, Pb-free solder](https://ecoquery.ecoinvent.org/3.12/cutoff/dataset/5084/documentation) | 源 `m2`；目标 `1 piece 存储背板子总成`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
| [Assembly line SMD (1SP, 2CS, 1CP, 1R, 1Rf) throughput 300/h - open input printed circuit board](https://lcadatabase.sphera.com/2026/xml-data/processes/309288a5-19a4-4efc-9e62-e5cf173751cc.xml) | 源 `kg`；目标 `1 piece 存储背板子总成`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
