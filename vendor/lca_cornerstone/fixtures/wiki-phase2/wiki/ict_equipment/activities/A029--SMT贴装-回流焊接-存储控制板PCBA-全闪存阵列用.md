...
id: A029
node_type: activity
display_name: "SMT贴装, 回流焊接 | 存储控制板PCBA, 全闪存阵列用"
boundary: foreground
facets: {transformation_verb: "smt_assembly", technology_route: "reflow_smt", reference_product_anchor: "ref::存储控制板PCBA, 全闪存阵列用"}
reference_product: "存储控制板PCBA, 全闪存阵列用"
produces: [P020, P030, P031]
...
spine_hash: "a17c31d866c8"
schema_version: wiki-v1
sigil: "sigil/A029.svg"
body_status: draft
...
表面贴装技术将元器件装配到印制板表面，回流焊用于形成其焊接互连；本节点将该工艺应用于全闪存阵列用存储控制板PCBA。 〔未核实·模型回忆〕

## 参考产品与参考单位

参考产品锚定为“存储控制板PCBA, 全闪存阵列用”；参考单位应以一块完成SMT贴装和回流焊接的控制板PCBA表示。 〔未核实·模型回忆〕 [^internal-review]
...
脊边对账应记录消耗P048、P040、P043、P044、P080、P081、P062、P066、P045和P087，并产出P020、P030及P031。 〔未核实·模型回忆〕 [^internal-review]
...
该节点有四项LCA关联且列出若干候选及已核验来源，但全闪存阵列控制板的实际配方、工艺能耗、良率和废物流数据仍需按工厂采集。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  P040["DRAM内存封装器件"] --> A029
  P043["HDI PCB裸板, 服务器/交换机用"] --> A029
  P044["被动元件, 电阻(片式)"] --> A029
  P045["连接器, 板对板"] --> A029
  P048["ASIC封装器件, 存储/NVMe控制器"] --> A029
  P062["无铅焊料锡膏, SAC305"] --> A029
  P066["中压电力, ICT制造用"] --> A029
  P080["被动元件, 电容(MLCC)"] --> A029
  P081["被动元件, 电感(片式)"] --> A029
  P087["板载存储连接器, SATA/SAS/U.2用"] --> A029
  A029(("SMT贴装, 回流焊接 | 存储控制板PCBA, 全闪存阵列用"))
  A029 ==> P020["存储控制板PCBA, 全闪存阵列用"]
  A029 --> P030["共生焊料浮渣"]
  A029 --> P031["共生报废PCBA"]
```
...
|---|---|---|---|---|
| [mounting, surface mount technology, Pb-free solder](https://ecoquery.ecoinvent.org/3.12/cutoff/dataset/5084/documentation) | 源 `m2`；目标 `1 piece 存储控制板PCBA, 全闪存阵列用`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
| [Assembly line SMD (1SP, 2CS, 1CP, 1R, 1Rf) throughput 300/h - open input printed circuit board](https://lcadatabase.sphera.com/2026/xml-data/processes/309288a5-19a4-4efc-9e62-e5cf173751cc.xml) | 源 `kg`；目标 `1 piece 存储控制板PCBA, 全闪存阵列用`；仍缺 4 项换算字段 | `partial`；待核 3 项 | 中国项目代理：锡膏 2.78 g/片；项目电力 1.39 kWh/片（均禁止计算） | C2 · 未形成模型绑定 · 不可直接计算 |
