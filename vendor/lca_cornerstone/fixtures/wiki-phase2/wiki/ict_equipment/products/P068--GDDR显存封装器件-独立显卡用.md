...
id: P068
node_type: product
display_name: "GDDR显存封装器件, 独立显卡用"
boundary: background
facets: {equipment_class: "compute_card", integration_level: "component", compute_type: "gpu_graphics_consumer", form_factor: "na", product_subtype: "gddr_memory"}
produced_by: []
consumed_by: [A041]
external: {cpc: "47160", hs: "8542.32"}
confidence: core
provenance_refs: [internal-review, ku-hiqlcd-public-ict-2026]
spine_hash: "1cd9c7d56be3"
schema_version: wiki-v1
sigil: "sigil/P068.svg"
body_status: draft
...
该节点表示用于独立显卡的GDDR显存封装器件，即符合图形双倍数据速率同步动态随机存取存储器用途的封装存储器件。 〔未核实·模型回忆〕

## 性质与形态

GDDR器件为同步DRAM，面向高带宽图形存储应用，并以封装器件形式安装在独立显卡电路板上。 〔未核实·模型回忆〕

## 参考流与交接边界

参考流应为一个完成封装并可交付给独立显卡装配活动的GDDR显存器件；不包括GPU、显卡电路板或整张显卡。 〔未核实·模型回忆〕 [^internal-review]

## 规格与相邻节点区分

本节点按GDDR存储器子类型、消费级图形用途和组件级集成度区分，不与通用DRAM、GPU封装器件或已装配计算卡合并。 〔未核实·模型回忆〕 [^internal-review]

## 在系统中的角色

GDDR同步DRAM为图形处理器提供专用的高带宽外部存储器，是独立显卡的关键功能部件。 〔未核实·模型回忆〕

## 分类与适用范围

按HS商品分类，电子集成电路归入品目8542；本节点的适用范围限于独立显卡用GDDR显存封装器件。 〔未核实·模型回忆〕

## 节点特定采集字段

应采集GDDR代际、容量、位宽、封装类型、单件质量、封装材料和晶圆制造及封装测试信息。 〔未核实·模型回忆〕 [^internal-review]

## 区域化补充要求

区域化时应分别记录存储器晶圆制造、封装测试和向显卡装配地运输的地区信息。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  P068["GDDR显存封装器件, 独立显卡用"]
  P068 --> A041(("SMT贴装, 回流焊接 | 消费级独立显卡PCBA"))
```
...
> 已核验关联 2 条：C0=0、C1=2、C2=0、C3=0、C4=0。这些记录用于发现与代理筛选；进入计算前仍须在具体项目中核对边界、许可和版本。
...
- **采取的修改：** 按冻结的官方/公开证据重新裁决本节点的数据集引用关系，当前展示 2 条关联（C0=0、C1=2、C2=0、C3=0、C4=0），另保留 0 条真正否决；每条同时标记关联对象、潜在用途、主要限制和项目裁决要求。
