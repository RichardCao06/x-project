...
id: P024
node_type: product
display_name: "GPU加速卡, AI训练用"
boundary: foreground
facets: {equipment_class: "compute_card", integration_level: "subsystem", compute_type: "gpu_ai_training", form_factor: "na", product_subtype: "accelerator"}
produced_by: [A003]
consumed_by: [A011, A012]
external: {cpc: "4529", hs: "8473.30"}
confidence: core
provenance_refs: [ecoinvent, gabi, hs, isic, ku-hiqlcd-public-ict-2026]
spine_hash: "291e9281d6e0"
schema_version: wiki-v1
sigil: "sigil/P024.svg"
body_status: draft
...
AI训练用GPU加速卡是集成GPU及其板级供电、存储与互连部件、面向训练工作负载的计算卡子系统。 〔未核实·模型回忆〕
...
本节点的参考流宜定义为一张完成制造并在出厂交付点可供服务器或系统集成使用的AI训练用GPU加速卡；上游芯片、存储器与板卡制造计入其供给，整机装配属于下游。 〔未核实·模型回忆〕 [^internal-review]

## 规格与相邻节点区分

该节点以compute_type=gpu_ai_training区分于AI推理用GPU加速卡；训练用途侧重大规模模型训练，而非仅部署后的推理服务。 〔未核实·模型回忆〕

## 在系统中的角色

该卡作为计算子系统由A003生产，并被A011和A012下游系统装配活动消耗。 〔未核实·模型回忆〕 [^internal-review]

## 分类与适用范围

该节点覆盖作为数据处理设备部件交付的AI训练GPU加速卡，不覆盖独立服务器整机或通用图形显示适配器。 〔未核实·模型回忆〕 [^internal-review]

## 节点特定采集字段

采集时应记录GPU型号与封装、板载存储器类型和容量、互连/主机接口、PCB尺寸与层数、散热方案及交付质量。 〔未核实·模型回忆〕 [^internal-review]

## 区域化补充要求

区域化应补充晶圆制造、先进封装、存储器和板卡装配的生产地及电力供应区域，并保留最终出货区域。 〔未核实·模型回忆〕 [^internal-review]

## 数据适用状态与缺口

现有档案未附经核验的来源，且缺少型号级物料构成、制造地点和训练卡专用配置；该节点仅可作为待补证的前景名称与采集框架使用。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  A003(("SMT贴装, 回流焊接 | GPU加速卡, AI训练用")) ==> P024
  P024["GPU加速卡, AI训练用"]
  P024 --> A011(("系统集成, 整机总装 | GPU服务器, AI训练, 4U"))
  P024 --> A012(("系统集成, 整机总装 | GPU服务器, AI推理, 2U"))
```
