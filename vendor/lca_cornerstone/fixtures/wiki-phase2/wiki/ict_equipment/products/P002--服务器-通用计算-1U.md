...
id: P002
node_type: product
display_name: "服务器, 通用计算, 1U"
boundary: foreground
facets: {equipment_class: "server", integration_level: "system", compute_type: "cpu_compute", form_factor: "1u", product_subtype: "通用服务器, 1u, cpu_compute"}
produced_by: [A010]
consumed_by: []
external: {cpc: "4521", hs: "8471.50"}
confidence: core
provenance_refs: [ecoinvent, hs, isic]
spine_hash: "b59521a7f9b4"
schema_version: wiki-v1
sigil: "sigil/P002.svg"
body_status: draft
...
该节点表示采用1U机架式外形、以CPU通用计算为主要计算方式的完整服务器系统。 〔未核实·模型回忆〕

## 性质与形态

1U服务器安装于标准19英寸机柜，其垂直安装高度为一个机架单位。 〔未核实·模型回忆〕

## 参考流与交接边界

参考流应为一台已完成装配并可交付的1U通用CPU服务器，交接边界设在服务器制造商的成品出厂点。 〔未核实·模型回忆〕 [^internal-review]

## 规格与相邻节点区分

该节点以1U外形和CPU通用计算身份区别于2U通用服务器、刀片服务器及以GPU推理为主的服务器。 〔未核实·模型回忆〕 [^internal-review]

## 在系统中的角色

该产品由对应的1U服务器制造活动产出，作为前景服务器产品在该图中不再指定下游消费活动。 〔未核实·模型回忆〕 [^internal-review]
...
采集应至少记录机架高度、CPU配置、内存、存储、主板、机箱、电源、散热部件和装配层级，以保持该节点的1U通用服务器身份。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  A010(("系统集成, 整机总装 | 服务器, 通用计算, 1U")) ==> P002
  P002["服务器, 通用计算, 1U"]
```
