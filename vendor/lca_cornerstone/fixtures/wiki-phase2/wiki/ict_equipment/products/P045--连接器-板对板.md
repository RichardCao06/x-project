...
id: P045
node_type: product
display_name: "连接器, 板对板"
boundary: background
facets: {equipment_class: "compute_card", integration_level: "component", compute_type: "na", form_factor: "na", product_subtype: "connector_board_to_board"}
produced_by: []
consumed_by: [A001, A002, A003, A004, A005, A024, A029, A030, A031]
external: {cpc: "4731", hs: "8536.69"}
confidence: core
provenance_refs: [internal-review, ku-hiqlcd-public-ict-2026, ku-ict-name-graph-20260730]
spine_hash: "5738d0dd74c2"
schema_version: wiki-v1
sigil: "sigil/P045.svg"
body_status: draft
...
P045“连接器, 板对板”是 ICT 名称图中的背景产品接口，不由 ICT 前景活动生产。当前跨行业解析指向 electronics 母产品“连接器, 板对板连接器”，`home_status=linked`；母行业负责维护生产机理和上游投入产出，ICT 页面只维护本行业接收时的身份、交接和用途。[^ku-ict-name-graph-20260730]

本节点的身份刻面为 `compute_type=na`；`equipment_class=compute_card`；`form_factor=na`；`integration_level=component`；`product_subtype=connector_board_to_board`。这些刻面用于区分图中的接口身份，不等同于厂商规格、性能等级或一条可计算 LCI。
...
上游 cradle-to-gate 清单应从 electronics 母产品“连接器, 板对板连接器” 或经项目裁决的数据集取得；ICT 前景从来料接收、检验或实际进入下游活动的交接点开始。不得同时计入母行业完整产品和其组成材料。〔建模规则〕
...
当前权威边显示本节点被以下 ICT 活动消费：A001“SMT贴装, 回流焊接 | 主板PCBA, 通用服务器用”、A002“SMT贴装, 回流焊接 | 主板PCBA, AI训练GPU服务器用”、A003“SMT贴装, 回流焊接 | GPU加速卡, AI训练用”、A004“SMT贴装, 回流焊接 | GPU加速卡, AI推理用”、A005“SMT贴装, 回流焊接 | 交换机主板PCBA, 100G/400G”、A024“SMT贴装, 回流焊接 | 交换机主板PCBA, 800G”、A029“SMT贴装, 回流焊接 | 存储控制板PCBA, 全闪存阵列用”、A030“SMT贴装, 回流焊接 | 存储控制板PCBA, 机械硬盘阵列用”、A031“SMT贴装, 回流焊接 | 存储背板子总成”。 〔未核实·模型回忆〕
...
- **产品身份：**供应商、制造商、完整料号、产品族、版本或代际，以及与 electronics 母产品“连接器, 板对板连接器” 的映射依据。
...
[^ku-ict-name-graph-20260730]: lca-cornerstone `docs/ict_equipment-name-graph.json` 中 P045 节点、相邻边和 `resolves_to`；本仓库权威名称图，按文件哈希核对。
<!-- BODY:END -->
...
flowchart LR
  P045["连接器, 板对板"]
  P045 --> A001(("SMT贴装, 回流焊接 | 主板PCBA, 通用服务器用"))
  P045 --> A002(("SMT贴装, 回流焊接 | 主板PCBA, AI训练GPU服务器用"))
  P045 --> A003(("SMT贴装, 回流焊接 | GPU加速卡, AI训练用"))
  P045 --> A004(("SMT贴装, 回流焊接 | GPU加速卡, AI推理用"))
  P045 --> A005(("SMT贴装, 回流焊接 | 交换机主板PCBA, 100G/400G"))
  P045 --> A024(("SMT贴装, 回流焊接 | 交换机主板PCBA, 800G"))
  P045 --> A029(("SMT贴装, 回流焊接 | 存储控制板PCBA, 全闪存阵列用"))
  P045 --> A030(("SMT贴装, 回流焊接 | 存储控制板PCBA, 机械硬盘阵列用"))
  P045 --> A031(("SMT贴装, 回流焊接 | 存储背板子总成"))
```
...
- **发现的问题：** P045 已有权威图身份、跨行业 GPID 和 LCA 数据集关联，但正文为空，无法说明接口边界、母行业归属和项目采集字段。
