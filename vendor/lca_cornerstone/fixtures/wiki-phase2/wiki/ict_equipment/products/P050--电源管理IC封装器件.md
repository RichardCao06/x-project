...
P050“电源管理IC封装器件”是 ICT 名称图中的背景产品接口，不由 ICT 前景活动生产。当前跨行业解析指向 electronics 母产品“封装器件, 模拟IC”，`home_status=linked`；母行业负责维护生产机理和上游投入产出，ICT 页面只维护本行业接收时的身份、交接和用途。[^ku-ict-name-graph-20260730]

本节点的身份刻面为 `compute_type=power_conditioning`；`equipment_class=psu_module`；`form_factor=na`；`integration_level=component`。这些刻面用于区分图中的接口身份，不等同于厂商规格、性能等级或一条可计算 LCI。
...
上游 cradle-to-gate 清单应从 electronics 母产品“封装器件, 模拟IC” 或经项目裁决的数据集取得；ICT 前景从来料接收、检验或实际进入下游活动的交接点开始。不得同时计入母行业完整产品和其组成材料。〔建模规则〕
...
当前权威边显示本节点被以下 ICT 活动消费：A001“SMT贴装, 回流焊接 | 主板PCBA, 通用服务器用”、A002“SMT贴装, 回流焊接 | 主板PCBA, AI训练GPU服务器用”、A003“SMT贴装, 回流焊接 | GPU加速卡, AI训练用”、A004“SMT贴装, 回流焊接 | GPU加速卡, AI推理用”、A005“SMT贴装, 回流焊接 | 交换机主板PCBA, 100G/400G”、A008“SMT贴装, 回流焊接 | PSU电源模组”、A024“SMT贴装, 回流焊接 | 交换机主板PCBA, 800G”、A041“SMT贴装, 回流焊接 | 消费级独立显卡PCBA”。 〔未核实·模型回忆〕
...
- **产品身份：**供应商、制造商、完整料号、产品族、版本或代际，以及与 electronics 母产品“封装器件, 模拟IC” 的映射依据。
...
[^ku-ict-name-graph-20260730]: lca-cornerstone `docs/ict_equipment-name-graph.json` 中 P050 节点、相邻边和 `resolves_to`；本仓库权威名称图，按文件哈希核对。
<!-- BODY:END -->
...
flowchart LR
  P050["电源管理IC封装器件"]
  P050 --> A001(("SMT贴装, 回流焊接 | 主板PCBA, 通用服务器用"))
  P050 --> A002(("SMT贴装, 回流焊接 | 主板PCBA, AI训练GPU服务器用"))
  P050 --> A003(("SMT贴装, 回流焊接 | GPU加速卡, AI训练用"))
  P050 --> A004(("SMT贴装, 回流焊接 | GPU加速卡, AI推理用"))
  P050 --> A005(("SMT贴装, 回流焊接 | 交换机主板PCBA, 100G/400G"))
  P050 --> A008(("SMT贴装, 回流焊接 | PSU电源模组"))
  P050 --> A024(("SMT贴装, 回流焊接 | 交换机主板PCBA, 800G"))
  P050 --> A041(("SMT贴装, 回流焊接 | 消费级独立显卡PCBA"))
```
...
| `C1` · 弱关联 | [electronic component production, active, unspecified](https://ecoquery.ecoinvent.org/3.12/cutoff/dataset/579/documentation) | ecoinvent 3.12 | `reference_product_and_producer_process` | 弱关联；可进入代理筛选 | 未通过节点特定硬门：reference_product、function、geography。未指定有源电子元件只能作为电源管理 IC 的产品族候选，未核实模拟/功率管理功能与封装测试路线。 | 仅知识关联 · 仍需项目裁决 · 不可直接计算 |
...
- **发现的问题：** P050 已有权威图身份、跨行业 GPID 和 LCA 数据集关联，但正文为空，无法说明接口边界、母行业归属和项目采集字段。
