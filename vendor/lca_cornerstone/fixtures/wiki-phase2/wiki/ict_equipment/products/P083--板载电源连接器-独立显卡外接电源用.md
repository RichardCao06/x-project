...
id: P083
node_type: product
display_name: "板载电源连接器, 独立显卡外接电源用"
boundary: background
facets: {equipment_class: "compute_card", integration_level: "component", compute_type: "gpu_graphics_consumer", form_factor: "na", product_subtype: "connector_external_power_board_mount"}
produced_by: []
consumed_by: [A041]
external: {cpc: "4731", hs: "8536.69"}
confidence: core
provenance_refs: [internal-review, ku-hiqlcd-public-ict-2026, ku-ict-name-graph-20260730]
spine_hash: "041eebd318d3"
schema_version: wiki-v1
sigil: "sigil/P083.svg"
body_status: draft
...
P083“板载电源连接器, 独立显卡外接电源用”是 ICT 名称图中的背景产品接口，不由 ICT 前景活动生产。当前跨行业解析指向 electronics 母产品“连接器, 线束连接器”，`home_status=linked`；母行业负责维护生产机理和上游投入产出，ICT 页面只维护本行业接收时的身份、交接和用途。[^ku-ict-name-graph-20260730]

本节点的身份刻面为 `compute_type=gpu_graphics_consumer`；`equipment_class=compute_card`；`form_factor=na`；`integration_level=component`；`product_subtype=connector_external_power_board_mount`。这些刻面用于区分图中的接口身份，不等同于厂商规格、性能等级或一条可计算 LCI。
...
共享同一母行业产品的 ICT 相邻接口包括：P088“线束连接器, ICT设备电源用”。 〔未核实·模型回忆〕
...
[^ku-ict-name-graph-20260730]: lca-cornerstone `docs/ict_equipment-name-graph.json` 中 P083 节点、相邻边和 `resolves_to`；本仓库权威名称图，按文件哈希核对。
<!-- BODY:END -->
...
```mermaid
flowchart LR
  P083["板载电源连接器, 独立显卡外接电源用"]
  P083 --> A041(("SMT贴装, 回流焊接 | 消费级独立显卡PCBA"))
```
...
- **发现的问题：** P083 已有权威图身份、跨行业 GPID 和 LCA 数据集关联，但正文为空，无法说明接口边界、母行业归属和项目采集字段。
