...
id: P020
node_type: product
display_name: "存储控制板PCBA, 全闪存阵列用"
boundary: foreground
facets: {equipment_class: "compute_card", integration_level: "subsystem", compute_type: "storage_flash", form_factor: "na", product_subtype: "controller_board"}
produced_by: [A029]
consumed_by: [A015, A032]
external: {cpc: "4522", hs: "8471.70"}
confidence: core
provenance_refs: [internal-review, ku-hiqlcd-public-ict-2026]
spine_hash: "5360486c4451"
schema_version: wiki-v1
sigil: "sigil/P020.svg"
body_status: draft
...
该节点表示集成于全闪存阵列中的存储控制板PCBA子系统，而非完整存储阵列或单颗闪存器件。 〔未核实·模型回忆〕
...
参考流宜定义为一块完成测试并可交付装配的全闪存阵列存储控制板PCBA，交接点设在板级制造完成之后。 〔未核实·模型回忆〕 [^internal-review]

## 规格与相邻节点区分

本节点按storage_flash计算类型区分于机械硬盘阵列用控制板PCBA，二者不应因同为控制板而合并。 〔未核实·模型回忆〕 [^internal-review]

## 在系统中的角色

该控制板PCBA作为全闪存阵列的上游板级子系统，被用于后续阵列装配活动。 〔未核实·模型回忆〕 [^internal-review]

## 分类与适用范围

该节点覆盖全闪存阵列应用的存储控制板PCBA，不覆盖机械硬盘阵列控制板或完整数据存储设备。 〔未核实·模型回忆〕 [^internal-review]

## 节点特定采集字段

采集应至少记录板级物料构成、元器件配置、制造与测试工序，以及面向全闪存阵列的接口或控制规格。 〔未核实·模型回忆〕 [^internal-review]
...
```mermaid
flowchart LR
  A029(("SMT贴装, 回流焊接 | 存储控制板PCBA, 全闪存阵列用")) ==> P020
  P020["存储控制板PCBA, 全闪存阵列用"]
  P020 --> A015(("系统集成, 整机总装 | 存储阵列, 全闪存, 4U"))
  P020 --> A032(("系统集成, 整机总装 | 存储阵列, 全闪存, 2U"))
```
